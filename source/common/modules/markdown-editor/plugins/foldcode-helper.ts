...
CodeMirror.registerHelper('fold', 'markdown', function (cm: CodeMirror.Editor, start: ReturnType<typeof CodeMirror.Pos>) {
  ...
  if (startLine.trimStart().startsWith("* ") || startLine.trimStart().startsWith("// "))
    return collapseBranch();

  if ((startLine.trimStart().startsWith("$ ") && startLine.endsWith("?")) || startLine.trimStart().startsWith("$ else") || startLine.trimStart().startsWith("$ elif"))
